...
NaiveErr naive_peq_init(NaivePeq *self, void *alloc_context, NaiveAllocFunc alloc, NaiveI32 num_bands_cap, NaiveI32 band_num_fos_cap, NaiveI32 band_num_sos_cap)
{
    ...
    self->bands = alloc(alloc_context, NAIVE_MEM_STATE, sizeof(NaiveCascadedIirDf1) * (NaiveUSize)num_bands_cap);
    if (!err && !self->bands)
        err = NAIVE_ERR_NOMEM;

    if (self->bands) {
        for (NaiveI32 i = 0; i < num_bands_cap; ++i) {
            NaiveErr rc = naive_cascaded_iir_df1_init(&self->bands[i], alloc_context, alloc, band_num_fos_cap, band_num_sos_cap);
            if (!err && rc)
                err = rc;
        }
    }
...
void naive_peq_process(NaivePeq *self, NaiveF32 *inout, NaiveI32 len)
{
    for (NaiveI32 i = 0; i < self->num_bands; ++i) {
        naive_cascaded_iir_df1_process(&self->bands[i], inout, len);
    }
}
...
{
    for (NaiveI32 i = 0; i < self->num_bands; ++i) {
        naive_cascaded_iir_df1_reset(&self->bands[i]);
    }
}
...
{
    for (NaiveI32 i = 0; i < self->num_bands; ++i) {
        naive_cascaded_iir_df1_set_default_params(&self->bands[i]);
    }
}

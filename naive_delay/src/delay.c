...
NaiveErr naive_delay_set_delay_len(NaiveDelay *self, NaiveI32 delay_len)
{
    ...
    if (delay_len > self->delay_len) {
        naive_delay_buf_write_zeros(&self->in_delay_buf, delay_len - self->delay_len);
        naive_delay_buf_write_zeros(&self->out_delay_buf, delay_len - self->delay_len);
    } else if (delay_len < self->delay_len) {
